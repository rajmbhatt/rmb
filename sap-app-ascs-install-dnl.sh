...
if [ ! -s /root/install/sap-app-ascs-install.sh ]
then
	echo "Download of /root/install/sap-app-ascs-install.sh file not successfull"
	echo "exiting..."
	...
	cd /root/install
	bash -x /root/install/sap-app-ascs-install.sh | tee -a /root/install/sap-app-ascs-install.sh-out.log
	if [ $? -ne 0 ] 
	then
		echo 1
		exit 1
